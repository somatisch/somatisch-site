...
(function($) {
  ...
  var SelectionSharer = function(options) {
    ...
    this.editUrl = $('meta[property="jk:src_url"]').attr("content") || $('meta[property="fb:src_url"]').attr("value");
    ...
    this.show = function(e) {
      setTimeout(function() {
        var sel = window.getSelection();
        var selection = self.getSelectionText(sel);
        if(!sel.isCollapsed && selection && selection.length>1) {
          var range = sel.getRangeAt(0);
    ...
    this.editUrlLink = function(e) {
      // var text = self.textSelection.replace(/<p[^>]*>/ig,'\n').replace(/<\/p>|  /ig,'').trim();

      // console.log(self.editUrl);

      $(this).attr("href",self.editUrl);
    ...
    this.render = function() {
      ...
      self.$popover.find('a.edit').click(self.editUrlLink);
      ...
      self.$popunder.find('a.edit').click(self.editUrlLink);
      ...
      if (self.editUrl){
        $(".selectionSharer a.edit").css('display','inline-block');
      }
    };
